...
STATE_PICK_YEAR = "pick_year"
STATE_PICK_ENGINE = "pick_engine"
STATE_VEHICLE_CONFIRMED = "vehicle_confirmed"
STATE_AWAITING_PART = "awaiting_part"

# In-memory sessions: user_id -> {state, vehicle_id, vehicle_name, vin, retry_count}
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        "\U0001F44B Bienvenue chez *PiecesAutoTN* \\!\n\n"
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if vin_str and VIN_PATTERN.match(vin_str.upper()):
...
async def handle_selection_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle brand/model/engine selection callbacks."""
    query = update.callback_query
    await query.answer()
    user_id = query.from_user.id
    session = _get_session(user_id)
    ...
    if data == "back_models":
        ...
        row = []
        for i, model in enumerate(models):
            label = model[:30] if len(model) > 30 else model
            row.append(InlineKeyboardButton(label, callback_data=f"model:{i}"))
            if len(row) == 2:
                keyboard.append(row)
                row = []
        if row:
            keyboard.append(row)
        keyboard.append([InlineKeyboardButton("\u2B05 Retour", callback_data="back_brands")])
        keyboard.append([InlineKeyboardButton("\U0001F504 Nouvelle recherche", callback_data="new_search")])
        await query.edit_message_text(
            f"\U0001F697 Selectionnez le modele {brand} :",
            reply_markup=InlineKeyboardMarkup(keyboard),
    ...
    if data == "back_years":
        ...
        row = []
        for y in years:
            row.append(InlineKeyboardButton(str(y), callback_data=f"year:{y}"))
            if len(row) == 3:
                keyboard.append(row)
                row = []
        if row:
        ...
        keyboard.append([InlineKeyboardButton("\U0001F504 Nouvelle recherche", callback_data="new_search")])
        await query.edit_message_text(
            f"\U0001F4C5 Selectionnez l'annee {brand} {model} :",
            reply_markup=InlineKeyboardMarkup(keyboard),
        )
        return
    ...
    if data == "back_engines":
        ...
        if year:
            ...
            from src.telegram.ui import grid_buttons
            items = [(_vehicle_short_label(v), f"engine:{v.id}") for v in vehicles]
            keyboard = grid_buttons(items, cols=2)
            keyboard.append([InlineKeyboardButton("\u2B05 Retour", callback_data="back_years")])
            keyboard.append([InlineKeyboardButton("\U0001F504 Nouvelle recherche", callback_data="new_search")])
            await query.edit_message_text(
    ...
    # Year selected — filter vehicles by year, then show engine if needed
    if data.startswith("year:"):
        year = int(data.split(":", 1)[1])
        session["year"] = year
        brand = session.get("brand", "")
        model = session.get("model", "")
        await _handle_year_selected(query, session, brand, model, year)
        return

    # Engine/vehicle selected
    if data.startswith("engine:"):
        ...
        # Notify operator of new VIN pattern if we have a VIN
        if session.get("vin"):
            await _auto_store_vin_pattern(session["vin"], vehicle_id, session["vehicle_name"])

        # If user already specified a part (from free text), resolve it
...
async def _handle_year_selected(query, session: dict, brand: str, model: str, year: int) -> None:
    """After year is selected, show engine choices or auto-select."""
    from src.db.repository import get_vehicles_for_model_year
    vehicles = await get_vehicles_for_model_year(brand, model, year)
    if not vehicles:
    ...
    if len(vehicles) == 1:
        ...
        await _show_vehicle_confirmed(query, session)
        return
    session["state"] = STATE_PICK_ENGINE
    from src.telegram.ui import grid_buttons
    items = [(_vehicle_short_label(v), f"engine:{v.id}") for v in vehicles]
    keyboard = grid_buttons(items, cols=2)
    keyboard.append([InlineKeyboardButton("\u2B05 Retour", callback_data="back_years")])
...
async def _show_parts_for_vehicle(query, session: dict) -> None:
    """Show available parts from DB for the client's selected vehicle."""
    from src.db.repository import get_parts_for_vehicle
    vehicle_id = session.get("vehicle_id")
    parts = await get_parts_for_vehicle(vehicle_id) if vehicle_id else []
    session["parts"] = parts
    ...
    for i, part in enumerate(parts):
        ...
        keyboard.append([InlineKeyboardButton(label, callback_data=f"part:{i}")])
    keyboard.append([InlineKeyboardButton("\U0001F504 Nouvelle recherche", callback_data="new_search")])
...
async def _match_part_to_db(vehicle_id: int, part_text: str) -> list[str]:
    ...
    """
    from src.db.repository import search_parts_fuzzy, get_parts_for_vehicle

    # Step 1: fuzzy SQL match
    matches = await search_parts_fuzzy(vehicle_id, part_text)
    if matches:
        return matches

    # Step 2: LLM interpretation against available parts
    all_parts = await get_parts_for_vehicle(vehicle_id)
    if not all_parts:
        return []
    ...
    if model:
        ...
        if matched_model:
            ...
            if not years:
                ...
                return True
            # If LLM extracted a year that matches DB, pre-select it
            if year and year in years:
                session["state"] = STATE_PICK_YEAR
                keyboard = [
                    [InlineKeyboardButton(f"\u2705 {year}", callback_data=f"year:{year}")],
                    [InlineKeyboardButton("\U0001F504 Nouvelle recherche", callback_data="new_search")],
                ]
                await update.message.reply_text(
...
async def _show_parts_for_vehicle_msg(update: Update, session: dict) -> None:
    """Show parts list via update.message (not query.edit)."""
    from src.db.repository import get_parts_for_vehicle
    vehicle_id = session.get("vehicle_id")
    parts = await get_parts_for_vehicle(vehicle_id) if vehicle_id else []
    session["parts"] = parts
...
def build_client_app() -> Application:
    ...
    app.add_handler(CallbackQueryHandler(handle_selection_callback, pattern=r"^(vin_yes|brand:|model:|year:|engine:|back_)"))
